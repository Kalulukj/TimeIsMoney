...
namespace TimeIsMoney.ViewModels
{
        ...
        } = new ObservableCollection<ColorCircle>()
    {
        ...
        [RelayCommand]
        public async void SaveNoteCategory()
        {
            var noteCategory = NoteCategory;
            if (String.IsNullOrWhiteSpace(noteCategory.name)) { return; }
            if (MyColor == null)
            {
                noteCategory.color = SelectedCategoryColor;
            }
            else
            {
                noteCategory.color = MyColor;
            }
            ...
            await App.NoteService.AddUpdateNoteCategoryAsync(noteCategory);
        ...
        [RelayCommand]
        private async void NoteCategoryTappedDelete(NoteCategory noteCategory)
        {
            if (noteCategory.nCategoryId == 0 || noteCategory == null)
            {
            ...
            if (noteCategory.nCategoryId > 0)
            {
                ...
                foreach (var item in newNoteList)
                {
                    if (item.nCategoryId == noteCategory.nCategoryId)
                    {
                        await App.NoteService.DeleteNoteAsync(item.noteId);
                    }
                }
            }
            await App.NoteService.DeleteNoteCategoryAsync(noteCategory.nCategoryId);
            await Shell.Current.GoToAsync("..");
        }
    }
}